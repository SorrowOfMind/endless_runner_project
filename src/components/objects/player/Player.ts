...
class Player extends GameObject implements PlayerInterface {
    ...
    constructor(game: GameInterface, x: number, y: number) {
        ...
        this.animation = new Animation(this.image, this.width, this.height, 0, 0, 6, this.maxFrameSpeed/this.game.speed);
        this.isRunning = true;
    }

    draw(ctx: CanvasRenderingContext2D) {
        if (this.isDucking) {
            this.animation.drawIrregularFrame(ctx, this.x, this.y, this.width, this.DUCK_HEIGHT, this.DUCK_OFFSET);
            return;
        }
    ...
    update() {
        ...
        if (this.game.keys.length === 0 && !this.isJumping) {
            this.isDucking = false;
            if (!this.isRunning) {
                this.animation.switchAnimation(0, this.SPRITE_MAX_FRAMES[0], this.maxFrameSpeed/this.game.speed);
                this.isRunning = true;
            }
        }
        ...
        if (this.game.keys.includes("down") && !this.isJumping) {
            if (!this.isDucking) {
                ...
                this.animation.switchAnimation(3, this.SPRITE_MAX_FRAMES[3], this.maxFrameSpeed/this.game.speed);
                this.isDucking = true;
            }
            this.isRunning = false;
        }
